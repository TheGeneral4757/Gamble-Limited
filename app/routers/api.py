...
def get_rate_limit() -> str:
    ...
    return (
        settings.rate_limit.game_requests
...
async def lottery_info():
    jackpot = await db.get_lottery_jackpot()
    jackpot_amount = jackpot["current_amount"]
    no_winner_months = jackpot["no_winner_months"]
    return lottery_system.get_lottery_info(jackpot_amount, no_winner_months)
...
async def lottery_buy(request: Request, data: LotteryBuyRequest):
    ...
    if count >= limit:
        raise HTTPException(
            400, f"You have reached the limit of {limit} tickets for this draw"
        )
...
async def lottery_draw_admin(request: Request):
    ...
    try:
        ...
        result = await lottery_system.perform_draw()
        ...
        return {
            "success": True,
            "message": f"Draw triggered. Winners: {safe_result['winners']}",
            "result": safe_result,
        }
