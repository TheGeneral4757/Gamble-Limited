...
def load_config() -> AppConfig:
    ...
    if get_env("ADMIN_LOGIN_PATH"):
        data.setdefault("security", {})["admin_login_path"] = get_env(
            "ADMIN_LOGIN_PATH"
        )
    ...
    if get_env("RATE_LIMIT_ENABLED"):
        data.setdefault("rate_limit", {})["enabled"] = get_env_bool(
            "RATE_LIMIT_ENABLED", True
        )
    if get_env("RATE_LIMIT_GAME_REQUESTS"):
        data.setdefault("rate_limit", {})["game_requests"] = get_env(
            "RATE_LIMIT_GAME_REQUESTS"
        )
    if get_env("RATE_LIMIT_API_REQUESTS"):
        data.setdefault("rate_limit", {})["api_requests"] = get_env(
    ...
    if "security" in data:
        ...
        if current_pwd and not (
            current_pwd.startswith("$2") and len(current_pwd) == 60
        ):
            try:
                print(
                    "Configuration: Detected plain text admin password. Hashing and updating config.json..."
                )
                hashed_bytes = bcrypt.hashpw(
                    current_pwd.encode("utf-8"), bcrypt.gensalt()
                )
                ...
                print(
                    "Configuration: Warning - Could not update config.json (Read-Only filesystem). Running with hashed password in memory only."
                )
