...
def run_game_tests():
    ...
    def test_plinko():
        ...
        assert "multiplier" in result or "slot" in result or "bucket" in result, "Missing multiplier/slot/bucket"
    ...
    def test_roulette():
        ...
        assert "number" in result or "result" in result or "winning_number" in result, "Missing number"
    ...
    def test_blackjack():
        ...
        assert "player_hand" in result or "player" in result, f"Missing player hand. Keys: {list(result.keys())}"
        # Dealer is shown as dealer_up_card during play or dealer_hand when complete
        assert "dealer_up_card" in result or "dealer_hand" in result or "dealer" in result, f"Missing dealer. Keys: {list(result.keys())}"
...
def run_websocket_tests():
    ...
    def test_chat_history():
        ...
        history = getattr(ws_manager, 'chat_history', getattr(ws_manager, '_chat_history', []))
